...
def get_dataset_statistics(
    ...
    num_transitions = 0
    num_trajectories = 0
    for traj in tqdm.tqdm(dataset.iterator(), total=cardinality if cardinality != tf.data.UNKNOWN_CARDINALITY else None,):
...
def normalize_action_and_proprio(
    ...
    if "proprio" in traj["observation"] and "proprio" in metadata:
