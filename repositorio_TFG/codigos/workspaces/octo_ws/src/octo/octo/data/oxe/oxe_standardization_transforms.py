...
def berkeley_autolab_ur5_dataset_transform(
    ...
    # make gripper action absolute action, +1 = open, 0 = close
    gripper_action = trajectory["action"][7]
    gripper_action = rel2abs_gripper_actions(gripper_action)
    ...
    trajectory["observation"]["proprio"] = trajectory["observation"]["robot_state"][:, 6:14]
    trajectory["language_instruction"] = trajectory["observation"]["natural_language_instruction"]
    return trajectory


def custom_ur5_none_proprio_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    # Reshape a (T, 7) if needed
    trajectory["action"] = tf.reshape(trajectory["action"], [-1, 7])

    # Reconstruct the action vector: [0:6] + gripper
    trajectory["action"] = tf.concat(
        [
            trajectory["action"][:, :6],                                    # xyz + rpy
            binarize_gripper_actions(trajectory["action"][:, -1])[:, None],   # + gripper (T, 1)
        ],
        axis=-1
    )
    return trajectory
...
def custom_ur5_euler_proprio_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    # Reshape a (T, 7) if needed
    #trajectory["action"] = tf.reshape(trajectory["action"], [-1, 7])
    ...
    trajectory["action"] = tf.concat(
        [
            trajectory["action"][:, 0:6],                                   # xyz + rpy
            binarize_gripper_actions(trajectory["action"][:, 6])[:, None],   # + gripper (T, 1)
        ],
        axis=-1
    )
...
def custom_ur5_joint_proprio_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    # Reshape a (T, 7) if needed
    #trajectory["action"] = tf.reshape(trajectory["action"], [-1, 7])
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    "custom_ur5_none_proprio": custom_ur5_none_proprio_dataset_transform,
    "custom_ur5_euler_proprio": custom_ur5_euler_proprio_dataset_transform,
    "custom_ur5_joint_proprio": custom_ur5_joint_proprio_dataset_transform,
